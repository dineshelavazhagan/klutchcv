...
class ball_det():
    ...
    def detect(self,source,save_img=False):
        weights,view_img, save_txt, self.imgsz, trace = opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace
    ...
    def create_new_video(self,frame_size,zoom_factor,file_name):
        ...
        out = cv2.VideoWriter(temp_full_path, cv2.VideoWriter_fourcc(*'XVID'), 30, tuple(frame_size))         
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        file_name = 'IMG_3853_2.mp4'
        source = "../data/raw/IMG_3853_2.mp4"
        ball_det = ball_det()

        # tr.run_tracking_per_frame(source)
        frame_size = ball_det.Suported_resolutions['high']
        ball_det.detect(source)
        new_vid = ball_det.create_new_video(frame_size,zoom_factor,file_name)
